...
public class MessageListener extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        boolean isAboveLaw = event.getMember() != null
                && event.getMember().getRoles().stream().anyMatch(r -> r.getId().equals("1488795130034000036"));
        ...
        if (forbidden != null) {
            ...
            event.getChannel().sendMessage("⚠️ <@" + event.getAuthor().getId()
                    + ">, your message was removed because it contained restricted language. Please maintain a professional environment.")
                    .delay(5, java.util.concurrent.TimeUnit.SECONDS)
                    .flatMap(net.dv8tion.jda.api.entities.Message::delete)
                    .queue(null, (err) -> {
                    });

            SupabaseClient.addWarning(event.getAuthor().getId(), event.getAuthor().getName(), "SYSTEM",
                    "Automated Filter", "Restricted Term: " + forbidden, event.getGuild().getId());
            ...
            if (logChannel != null) {
                ...
                String logBody = "### 🛡️ RESTRICTED WORD DETECTED\n" +
                        "**Under surveillance:** <@" + event.getAuthor().getId() + ">\n" +
                ...
                PanelService.reply(logChannel, logEmbed);
            }
            return;
        }
        ...
        if (autoReply != null) {
            event.getMessage().reply(autoReply).setAllowedMentions(
                    java.util.Arrays.asList(net.dv8tion.jda.api.entities.Message.MentionType.values())).queue();
            return;
        }
