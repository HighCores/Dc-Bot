...
public class CentralInteractionListener extends ListenerAdapter {
    ...
    public void onStringSelectInteraction(StringSelectInteractionEvent event) {
        if (!event.isAcknowledged()) {
            String id = event.getComponentId();
            // Order flow and info panels → ephemeral
            boolean ephemeral = id.startsWith("order_") || id.equals("view_services_cat")
                             || id.equals("view_prices_cat") || id.equals("ticket_type_select");
            if (ephemeral) {
    ...
    private void processSelect(StringSelectInteractionEvent event) {
        try {
            ...
            else if (id.equals("ticket_type_select"))        { TicketService.createTicket(event, "General Request", "MEDIUM", val); }
    ...
    private void handleServiceDisplay(StringSelectInteractionEvent event, String val) {
        ...
        PanelService.reply(event, EmbedUtil.containerBrandedRows("SERVICES", "Active Capabilities", body,
                EmbedUtil.BANNER_MAIN, ActionRow.of(Button.success("order_start", "Start Order"))));
    }
