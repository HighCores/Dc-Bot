package com.highcore.bot.commands;

import com.google.gson.JsonArray;
import com.google.gson.JsonObject;
import com.highcore.bot.config.Config;
import com.highcore.bot.database.SupabaseClient;
import com.highcore.bot.services.GiveawayService;
import com.highcore.bot.utils.EmbedUtil;
import net.dv8tion.jda.api.entities.Member;
import net.dv8tion.jda.api.entities.channel.concrete.TextChannel;
import net.dv8tion.jda.api.events.interaction.ModalInteractionEvent;
import net.dv8tion.jda.api.events.interaction.command.SlashCommandInteractionEvent;
import net.dv8tion.jda.api.events.interaction.component.ButtonInteractionEvent;
import net.dv8tion.jda.api.events.interaction.component.StringSelectInteractionEvent;
import net.dv8tion.jda.api.hooks.ListenerAdapter;
import net.dv8tion.jda.api.interactions.commands.OptionMapping;
import net.dv8tion.jda.api.interactions.components.ActionRow;
import net.dv8tion.jda.api.interactions.components.buttons.Button;
import net.dv8tion.jda.api.interactions.components.selections.StringSelectMenu;
import net.dv8tion.jda.api.interactions.components.text.TextInput;
import net.dv8tion.jda.api.interactions.components.text.TextInputStyle;
import net.dv8tion.jda.api.interactions.modals.Modal;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.time.Instant;
import java.time.temporal.ChronoUnit;

public class GiveawayCommands extends ListenerAdapter {
    private static final Logger log = LoggerFactory.getLogger(GiveawayCommands.class);

    @Override
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        if (!isAdmin(event.getMember())) return;
        switch (event.getName()) {
            case "giveaway-start" -> handleStart(event);
            case "giveaway-end" -> handleEnd(event);
            case "giveaway-reroll" -> handleReroll(event);
            case "giveaway-list" -> handleList(event);
            case "giveaway-drop" -> handleDrop(event);
            case "giveaway-panel" -> handlePanel(event);
        }
    }

    // ===== PANEL =====
    private void handlePanel(SlashCommandInteractionEvent event) {
        event.replyEmbeds(EmbedUtil.branded().setColor(EmbedUtil.GOLD)
                .setDescription("""
                        ## \uD83C\uDF89 Giveaway Management Panel

                        > Welcome to the Giveaway system! Here you can create and manage giveaways for your community.

                        **How it works:**
                        > \u25B8 Click **Create Giveaway** to start a new one
                        > \u25B8 Choose the prize type (discount, coupon, free service, etc.)
                        > \u25B8 Fill in the details and select where to post it
                        > \u25B8 Members join by clicking the \uD83C\uDF89 button
                        > \u25B8 Winners are picked automatically when time runs out

                        **Quick commands:**
                        > `/giveaway-end` \u2014 End a giveaway early
                        > `/giveaway-reroll` \u2014 Pick new winners
                        > `/giveaway-list` \u2014 View active giveaways
                        """).build())
                .addComponents(ActionRow.of(
                        Button.success("gw_create", "\u2795 Create Giveaway"),
                        Button.secondary("gw_list", "\uD83D\uDCCB List Active")))
                .queue();
    }

    // ===== PANEL BUTTONS =====
    @Override
    public void onButtonInteraction(ButtonInteractionEvent event) {
        if (!event.getComponentId().startsWith("gw_")) return;
        if (!isAdmin(event.getMember())) { event.reply("Admin only.").setEphemeral(true).queue(); return; }

        switch (event.getComponentId()) {
            case "gw_create" -> {
                // Show type selector
                event.reply("\uD83C\uDF81 **Select giveaway prize type:**")
                        .addComponents(ActionRow.of(
                                StringSelectMenu.create("gw_type_select")
                                        .setPlaceholder("Choose prize type...")
                                        .addOption("\uD83D\uDCB0 Discount", "discount", "Percentage or fixed discount")
                                        .addOption("\uD83C\uDF9F\uFE0F Coupon", "coupon", "Gift card or coupon code")
                                        .addOption("\uD83C\uDD93 Free Service", "free_service", "A free service offering")
                                        .addOption("\uD83C\uDF81 Custom Prize", "custom", "Any other prize")
                                        .build()))
                        .setEphemeral(true).queue();
            }
            case "gw_list" -> {
                JsonArray active = SupabaseClient.getActiveGiveaways();
                if (active == null || active.size() == 0) {
                    event.replyEmbeds(EmbedUtil.info("Giveaways", "No active giveaways.")).setEphemeral(true).queue();
                    return;
                }
                StringBuilder sb = new StringBuilder();
                for (var el : active) {
                    JsonObject g = el.getAsJsonObject();
                    sb.append("`#").append(g.get("id").getAsLong()).append("` \u2014 ")
                            .append(g.has("prize_details") ? g.get("prize_details").getAsString() : "?")
                            .append(" \u2014 <#").append(g.get("channel_id").getAsString()).append(">\n");
                }
                event.replyEmbeds(EmbedUtil.base().setColor(EmbedUtil.GOLD)
                        .setDescription("### \uD83C\uDF89 Active Giveaways\n" + sb).build()).setEphemeral(true).queue();
            }
        }
    }

    // ===== TYPE SELECTOR =====
    @Override
    public void onStringSelectInteraction(StringSelectInteractionEvent event) {
        if (!event.getComponentId().equals("gw_type_select")) return;
        String type = event.getValues().get(0);

        TextInput channel = TextInput.create("gw_channel", "Channel ID (where to post)", TextInputStyle.SHORT)
                .setPlaceholder("Right-click channel → Copy Channel ID").setRequired(true).build();
        TextInput duration = TextInput.create("gw_duration", "Duration (in minutes)", TextInputStyle.SHORT)
                .setPlaceholder("e.g. 60 = 1 hour, 1440 = 1 day").setRequired(true).build();
        TextInput winners = TextInput.create("gw_winners", "Number of winners", TextInputStyle.SHORT)
                .setPlaceholder("1").setRequired(true).setValue("1").build();

        switch (type) {
            case "discount" -> {
                TextInput details = TextInput.create("gw_details", "Discount details (service, %, max)", TextInputStyle.PARAGRAPH)
                        .setPlaceholder("e.g. 30% off Web Development, up to $100").setRequired(true).build();
                event.replyModal(Modal.create("gw_modal_discount", "\uD83D\uDCB0 Discount Giveaway")
                        .addComponents(ActionRow.of(details), ActionRow.of(channel), ActionRow.of(duration), ActionRow.of(winners)).build()).queue();
            }
            case "coupon" -> {
                TextInput details = TextInput.create("gw_details", "Coupon details (value, currency, expiry)", TextInputStyle.PARAGRAPH)
                        .setPlaceholder("e.g. $50 USD coupon, expires Dec 2025").setRequired(true).build();
                event.replyModal(Modal.create("gw_modal_coupon", "\uD83C\uDF9F\uFE0F Coupon Giveaway")
                        .addComponents(ActionRow.of(details), ActionRow.of(channel), ActionRow.of(duration), ActionRow.of(winners)).build()).queue();
            }
            case "free_service" -> {
                TextInput details = TextInput.create("gw_details", "Service details (what, where, value)", TextInputStyle.PARAGRAPH)
                        .setPlaceholder("e.g. Free Discord Bot setup (worth $200)").setRequired(true).build();
                event.replyModal(Modal.create("gw_modal_free_service", "\uD83C\uDD93 Free Service Giveaway")
                        .addComponents(ActionRow.of(details), ActionRow.of(channel), ActionRow.of(duration), ActionRow.of(winners)).build()).queue();
            }
            case "custom" -> {
                TextInput details = TextInput.create("gw_details", "Prize description", TextInputStyle.PARAGRAPH)
                        .setPlaceholder("Describe the prize...").setRequired(true).build();
                event.replyModal(Modal.create("gw_modal_custom", "\uD83C\uDF81 Custom Giveaway")
                        .addComponents(ActionRow.of(details), ActionRow.of(channel), ActionRow.of(duration), ActionRow.of(winners)).build()).queue();
            }
        }
    }

    // ===== MODAL SUBMIT =====
    @Override
    public void onModalInteraction(ModalInteractionEvent event) {
        if (!event.getModalId().startsWith("gw_modal_")) return;
        event.deferReply(true).queue();

        String type = event.getModalId().replace("gw_modal_", "");
        String details = event.getValue("gw_details").getAsString();
        String channelId = event.getValue("gw_channel").getAsString().trim();
        int durationMin;
        int winnerCount;
        try {
            durationMin = Integer.parseInt(event.getValue("gw_duration").getAsString().trim());
            winnerCount = Integer.parseInt(event.getValue("gw_winners").getAsString().trim());
        } catch (NumberFormatException e) {
            event.getHook().editOriginalEmbeds(EmbedUtil.error("Error", "Duration and winners must be numbers.")).queue();
            return;
        }

        TextChannel targetCh = event.getGuild().getTextChannelById(channelId);
        if (targetCh == null) {
            event.getHook().editOriginalEmbeds(EmbedUtil.error("Error", "Channel not found. Make sure you copied the correct ID.")).queue();
            return;
        }

        String endsAt = Instant.now().plus(durationMin, ChronoUnit.MINUTES).toString();
        String typeEmoji = switch (type) {
            case "discount" -> "\uD83D\uDCB0"; case "coupon" -> "\uD83C\uDF9F\uFE0F";
            case "free_service" -> "\uD83C\uDD93"; default -> "\uD83C\uDF81";
        };
        String typeName = switch (type) {
            case "discount" -> "Discount"; case "coupon" -> "Coupon";
            case "free_service" -> "Free Service"; default -> "Custom Prize";
        };

        // Create in DB
        JsonObject giveaway = SupabaseClient.createGiveaway(channelId, event.getGuild().getId(),
                event.getUser().getId(), event.getUser().getName(), type, details, null, null, null, null, winnerCount, endsAt);

        if (giveaway == null) {
            event.getHook().editOriginalEmbeds(EmbedUtil.error("Error", "Failed to create giveaway.")).queue();
            return;
        }

        long giveawayId = giveaway.get("id").getAsLong();
        String timeStr = durationMin >= 1440 ? (durationMin / 1440) + " day(s)" : durationMin >= 60 ? (durationMin / 60) + " hour(s)" : durationMin + " min";

        // Send giveaway message
        targetCh.sendMessageEmbeds(EmbedUtil.base().setColor(EmbedUtil.GOLD)
                .setDescription("## \uD83C\uDF89 GIVEAWAY " + typeEmoji + "\n\n" +
                        "> **" + details + "**\n\n" +
                        "**Type:** " + typeName + "\n" +
                        "**Winners:** " + winnerCount + "\n" +
                        "**Duration:** " + timeStr + "\n" +
                        "**Hosted by:** " + event.getUser().getAsMention() + "\n\n" +
                        "Click the button below to enter! \uD83C\uDF89")
                .build()).addComponents(ActionRow.of(
                        Button.success("gw_enter_" + giveawayId, "\uD83C\uDF89 Enter Giveaway"),
                        Button.secondary("gw_count_" + giveawayId, "0 entries")))
                .queue(msg -> {
                    SupabaseClient.setGiveawayMessageId(giveawayId, msg.getId());
                });

        event.getHook().editOriginalEmbeds(EmbedUtil.success("Giveaway Created",
                "Giveaway **#" + giveawayId + "** created in " + targetCh.getAsMention() + "!\nEnds in **" + timeStr + "**")).queue();
    }

    // ===== SLASH COMMANDS =====
    private void handleStart(SlashCommandInteractionEvent event) {
        String prize = event.getOption("prize", OptionMapping::getAsString);
        int duration = event.getOption("duration", 60, OptionMapping::getAsInt);
        int winners = event.getOption("winners", 1, OptionMapping::getAsInt);
        TextChannel ch = event.getOption("channel") != null ? event.getOption("channel", OptionMapping::getAsChannel).asTextChannel() : event.getChannel().asTextChannel();

        String endsAt = Instant.now().plus(duration, ChronoUnit.MINUTES).toString();
        JsonObject g = SupabaseClient.createGiveaway(ch.getId(), event.getGuild().getId(),
                event.getUser().getId(), event.getUser().getName(), "custom", prize, null, null, null, null, winners, endsAt);
        if (g == null) { event.replyEmbeds(EmbedUtil.error("Error", "Failed to create.")).setEphemeral(true).queue(); return; }

        long gid = g.get("id").getAsLong();
        String timeStr = duration >= 60 ? (duration / 60) + "h" : duration + "min";
        ch.sendMessageEmbeds(EmbedUtil.base().setColor(EmbedUtil.GOLD)
                .setDescription("## \uD83C\uDF89 GIVEAWAY\n\n> **" + prize + "**\n\n**Winners:** " + winners +
                        "\n**Duration:** " + timeStr + "\n**Hosted by:** " + event.getUser().getAsMention() +
                        "\n\nClick to enter! \uD83C\uDF89").build())
                .addComponents(ActionRow.of(
                        Button.success("gw_enter_" + gid, "\uD83C\uDF89 Enter"),
                        Button.secondary("gw_count_" + gid, "0 entries")))
                .queue(msg -> SupabaseClient.setGiveawayMessageId(gid, msg.getId()));
        event.replyEmbeds(EmbedUtil.success("Giveaway", "Created in " + ch.getAsMention())).setEphemeral(true).queue();
    }

    private void handleEnd(SlashCommandInteractionEvent event) {
        String idStr = event.getOption("id", OptionMapping::getAsString);
        try {
            long id = Long.parseLong(idStr);
            GiveawayService.endGiveaway(event.getJDA(), id, 1);
            event.replyEmbeds(EmbedUtil.success("Ended", "Giveaway **#" + id + "** ended.")).setEphemeral(true).queue();
        } catch (Exception e) { event.replyEmbeds(EmbedUtil.error("Error", "Invalid ID.")).setEphemeral(true).queue(); }
    }

    private void handleReroll(SlashCommandInteractionEvent event) {
        String idStr = event.getOption("id", OptionMapping::getAsString);
        try {
            long id = Long.parseLong(idStr);
            GiveawayService.rerollGiveaway(event.getJDA(), id);
            event.replyEmbeds(EmbedUtil.success("Rerolled", "Giveaway **#" + id + "** rerolled.")).setEphemeral(true).queue();
        } catch (Exception e) { event.replyEmbeds(EmbedUtil.error("Error", "Invalid ID.")).setEphemeral(true).queue(); }
    }

    private void handleList(SlashCommandInteractionEvent event) {
        JsonArray active = SupabaseClient.getActiveGiveaways();
        if (active == null || active.size() == 0) { event.replyEmbeds(EmbedUtil.info("Giveaways", "No active giveaways.")).setEphemeral(true).queue(); return; }
        StringBuilder sb = new StringBuilder();
        for (var el : active) {
            JsonObject g = el.getAsJsonObject();
            sb.append("`#").append(g.get("id").getAsLong()).append("` \u2014 ")
                    .append(g.has("prize_details") ? g.get("prize_details").getAsString() : "?")
                    .append(" \u2014 <#").append(g.get("channel_id").getAsString()).append(">\n");
        }
        event.replyEmbeds(EmbedUtil.base().setColor(EmbedUtil.GOLD).setDescription("### \uD83C\uDF89 Active Giveaways\n" + sb).build()).setEphemeral(true).queue();
    }

    private void handleDrop(SlashCommandInteractionEvent event) {
        String prize = event.getOption("prize", OptionMapping::getAsString);
        TextChannel ch = event.getOption("channel") != null ? event.getOption("channel", OptionMapping::getAsChannel).asTextChannel() : event.getChannel().asTextChannel();
        // Drop = first to click wins, 1 minute duration
        String endsAt = Instant.now().plus(1, ChronoUnit.MINUTES).toString();
        JsonObject g = SupabaseClient.createGiveaway(ch.getId(), event.getGuild().getId(),
                event.getUser().getId(), event.getUser().getName(), "drop", prize, null, null, null, null, 1, endsAt);
        if (g == null) { event.replyEmbeds(EmbedUtil.error("Error", "Failed.")).setEphemeral(true).queue(); return; }
        long gid = g.get("id").getAsLong();
        ch.sendMessageEmbeds(EmbedUtil.base().setColor(EmbedUtil.GOLD)
                .setDescription("## \uD83D\uDCA8 DROP GIVEAWAY!\n\n> **" + prize + "**\n\n\u26A1 First to click wins! Be fast!").build())
                .addComponents(ActionRow.of(Button.success("gw_enter_" + gid, "\uD83D\uDCA8 CLAIM!")))
                .queue(msg -> SupabaseClient.setGiveawayMessageId(gid, msg.getId()));
        event.replyEmbeds(EmbedUtil.success("Drop", "Drop giveaway sent in " + ch.getAsMention())).setEphemeral(true).queue();
    }

    private boolean isAdmin(Member m) { return m != null && m.getRoles().stream().anyMatch(r -> Config.getAdminRoles().contains(r.getId())); }
}
